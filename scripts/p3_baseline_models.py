...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train one baseline model from merged inputs.")
    parser.add_argument("--annotations-csv", default="data/annotations/leu_t_transporters.csv")
    parser.add_argument("--features-csv", required=True)
    parser.add_argument("--family", default=None)
    parser.add_argument("--model", choices=["logreg", "random_forest", "svm_rbf"], default="logreg")
...
def main() -> None:
    args = parse_args()
    available = sorted(get_baseline_models(random_state=args.random_state).keys())
