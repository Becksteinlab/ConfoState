...
def main():
    ...
    if os.path.exists(annotations_path):
        ...
        print(f"\n   First 5 entries:")
        print(df_annot[["pdb_id", "conformation", "experimental_method", "year"]].head())
    else:
    ...
    if os.path.isdir(input_dir):
        ...
        if len(df_input) > 0:
            ...
            print(f"   Download structures using:")
            print(f"      python scripts/download_structures.py --codes-file data/protein_families/LeuT_transporters.txt --output-dir {input_dir}")
    else:
